...
public class Setup {
	...
	public void run() { // start the setup by retracing all points (root retracers) on the screen
		...
		for (int i = 0; i < screen.retracerRoots.size(); i++) {
			...
			// each thread gets a workload no larger than the batch size
			if ((i + 1) % batchSize == 0 || i == screen.retracerRoots.size() - 1) {
		...
		for (Thread thread : threads) { // wait for all threads to finish execution
			try {
				...
				JOptionPane.showMessageDialog(null, "Calculation thread exception\n" + e.toString());
			}
		}
	...
	public void addComponent(String componentName, Map<String, Object> properties) {
		...
		try {
			component = (Component) Class.forName("components.C_" + componentName).getConstructor().newInstance();
		} catch (Exception e) {
			Main.quitWithMessage("Component <" + componentName + "> could not be instanced\n" + e.toString());
		}
		// set properties
		for (Map.Entry<String, Object> entry : properties.entrySet()) {
			try {
				component.getClass().getField(entry.getKey()).set(component, entry.getValue());
			} catch (Exception e) {
				Main.quitWithMessage("Property <" + entry.getKey() + "> could not be set for component <" + componentName + ">\n" + e.toString());
			}
		}
		component.angle = Lis.normalizeAngle(component.angle); // this is needed for other procedures
		if (component instanceof C_Screen) {
			screen = (C_Screen) component;
		}
