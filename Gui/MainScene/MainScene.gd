...
func _unhandled_input(event):
	if event is InputEventMouseButton: 
		if event.button_index == BUTTON_LEFT: dragging = event.pressed #start or end dragging
		#zoom in or out
...
func _ready(): #add initial screen component
...
#a cache for tracking symbol nodes
